...
open class IdeaCommunityProperties(private val communityHomeDir: Path) : BaseIdeaProperties() {
  ...
  protected open fun bundleExternalPlugins(context: BuildContext, targetDirectory: String) {
    ...
    deployAllPlugins(targetDirectory);
  }

  private val pathToBundlePluginsFolder = communityHomeDir.resolve("build/bundle_plugins")

  private fun deployAllPlugins(targetDirectory: String) {

    val folder = pathToBundlePluginsFolder
    val archive = folder.toFile().listFiles()!!.forEach {
      if(it.name.endsWith(".zip")) {
        ExternalPluginBundler.extractPlugin(it.toPath(), targetDirectory)
      }
    }
  }
